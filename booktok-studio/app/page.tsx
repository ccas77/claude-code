...
import type { Book, ModuleOutput } from "./lib/types";
import { renderAll } from "./lib/catalog";
...
export default function Page() {
  const [book, setBook] = useState<Book>(SAMPLE_BOOKS[0]);
  const [outputs, setOutputs] = useState<ModuleOutput[]>(() => renderAll(SAMPLE_BOOKS[0]));

  function generate() {
    setOutputs(renderAll(book));
  }
    ...
    <main style={{ display: "flex", minHeight: "100vh", fontFamily: "system-ui, sans-serif" }}>
      <aside style={{ width: 380, padding: 24, borderRight: "1px solid #2a2a2a", background: "#141414", color: "#eee", overflowY: "auto" }}>
        ...
        <div style={{ display: "flex", gap: 6, margin: "12px 0" }}>
          {SAMPLE_BOOKS.map((b) => (
            <button key={b.title} onClick={() => { setBook(b); setOutputs(renderAll(b)); }} style={chip}>{b.title}</button>
          ))}
        </div>
        ...
        <button onClick={generate} style={cta}>Generate all outputs →</button>
      </aside>
      <section style={{ flex: 1, padding: 24, background: "#0d0d0d", color: "#eee", overflowY: "auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(240px,1fr))", gap: 20 }}>
          {outputs.map((o) => <OutputCard key={o.moduleKey} out={o} />)}
        </div>
      </section>
    </main>
  );
  ...
  return (
    <div style={{ border: "1px solid #2a2a2a", borderRadius: 10, overflow: "hidden", background: "#161616" }}>
      <div style={{ padding: "8px 10px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <span style={{ fontSize: 10, color: out.implemented ? "#4caf50" : "#e0a52b" }}>{out.implemented ? "live" : "storyboard"}</span>
      </div>
      <div style={{ fontSize: 10, color: "#777", padding: "0 10px 6px" }}>from {out.sourceApp}</div>
      {slide && <img src={svgDataUrl(slide.svg)} alt={slide.label} style={{ width: "100%", display: "block", background: "#000", aspectRatio: out.aspect.replace(":", "/") }} />}
      <div style={{ display: "flex", gap: 4, flexWrap: "wrap", padding: 8 }}>
        {out.slides.map((s, idx) => (
          <button key={idx} onClick={() => setI(idx)} style={{ ...tab, ...(idx === i ? tabActive : {}) }}>{s.label}</button>
        ))}
      </div>
      {out.note && <div style={{ fontSize: 10, color: "#e0a52b", padding: "0 10px 8px" }}>{out.note}</div>}
      <div style={{ fontSize: 11, color: "#aaa", padding: "0 10px 10px", whiteSpace: "pre-wrap", borderTop: "1px solid #222", paddingTop: 8 }}>{out.caption}</div>
    </div>
  );
}
